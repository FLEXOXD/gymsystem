            ...
            <?php if($screen === 'home'): ?>
                <button id="leaderboard-toggle" type="button" class="leaderboard-chip" aria-haspopup="dialog" aria-expanded="false" aria-controls="leaderboard-modal">
                    ...
                    <span class="leaderboard-chip-copy">
                        ...
                        <span id="leaderboard-chip-status" class="leaderboard-chip-status">Compite esta semana</span>
                    </span>
                </button>
            <?php endif; ?>
                ...
                <div class="leaderboard-modal-dialog" role="dialog" aria-modal="true" aria-labelledby="leaderboard-title">
                    <article class="leaderboard-card">
                        <div class="flex items-start justify-between gap-3">
                            <div>
                                <p class="leaderboard-tag">Corona semanal</p>
                                <h3 id="leaderboard-title" class="leaderboard-title">Top 5 disciplina</h3>
                        ...
                        <p id="leaderboard-window" class="leaderboard-window">Semana actual</p>
                        <p id="leaderboard-helper" class="leaderboard-helper">La asistencia pesa mas y el tiempo solo suma como extra segun tu objetivo personal.</p>
                        <div id="leaderboard-top-list" class="leaderboard-list"></div>
                        <p id="leaderboard-empty" class="leaderboard-empty hidden">Todavia no hay puntajes suficientes para mostrar el Top 5 de esta semana.</p>
                        <article id="leaderboard-current-card" class="leaderboard-current-card hidden">
                            ...
                            <p id="leaderboard-current-meta" class="leaderboard-current-meta">Cuando sumes sesiones validas esta semana aparecera tu posicion aqui.</p>
...
(function () {
    ...
    function renderLeaderboard(progressPayload) {
        ...
        if (leaderboardChipStatusEl) {
            ...
            leaderboardChipStatusEl.textContent = chipLabel !== '' ? chipLabel : 'Compite esta semana';
        }
        if (leaderboardWindowEl) {
            const windowLabel = String(leaderboard.window_label || '').trim();
            leaderboardWindowEl.textContent = windowLabel !== '' ? windowLabel : 'Semana actual';
        }
        if (leaderboardHelperEl) {
            const helperLabel = String(leaderboard.helper || '').trim();
            leaderboardHelperEl.textContent = helperLabel !== '' ? helperLabel : 'La asistencia pesa mas y el tiempo solo suma como extra segun tu objetivo personal.';
        }
        ...
        if (currentClient && !currentClientInTop) {
            ...
            leaderboardCurrentHintEl.textContent = 'Sigue sumando asistencias validas para meterte al Top 5.';
            return;
        }
        ...
        if (!currentClient) {
            ...
            leaderboardCurrentMetaEl.textContent = 'Cuando registres una sesion valida esta semana apareceras con tu posicion actual.';
            leaderboardCurrentHintEl.textContent = 'La asistencia pesa mas y el tiempo te da un empuje extra.';
            return;
        }
