...
<style>
    @media (max-width: 1023px) {
        ...
        .theme-light .mnav-btn-active {
            border-color: color-mix(in srgb, var(--accent) 40%, #bfa167);
            background: linear-gradient(145deg, color-mix(in srgb, var(--primary) 16%, #fff7e3), color-mix(in srgb, var(--accent) 12%, #ffffff));
            color: #6b4f1d;
            box-shadow: 0 10px 24px rgba(180, 138, 61, 0.18);
        }
        ...
        .theme-light .mnav-btn-highlight {
            ...
            background: linear-gradient(145deg, rgba(220, 252, 231, 0.98), rgba(240, 253, 244, 0.94));
            color: #166534;
        }
...
<?php if($mobileHasOverflow): ?>
    ...
    <section id="mnav-sheet" class="mnav-sheet hidden lg:hidden" role="dialog" aria-modal="true" aria-label="Módulos">
        ...
        <div class="mnav-sheet-grid">
            <?php $__currentLoopData = $mobileOverflowItems; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <?php
                ...
                <a href="<?php echo e(route($item['route'], $item['params'] ?? [])); ?>"
                   class="mnav-sheet-link <?php echo e(trim($linkClass)); ?>">
                    <span><?php echo e($item['label']); ?></span>
                    <?php if($mobileIsSupportChatInbox((array) $item)): ?>
                        <span data-support-chat-badge
                              data-count="<?php echo e($mobileSupportChatUnread); ?>"
                              class="mnav-chat-badge <?php echo e($mobileSupportChatUnread > 0 ? '' : 'hidden'); ?>">
                            <?php echo e(min(99, $mobileSupportChatUnread)); ?>
...
<nav id="mnav-root" class="mnav-shell theme-mobile-nav lg:hidden" data-mobile-nav="1">
    <div class="mnav-grid" style="grid-template-columns: repeat(<?php echo e($mobileHasOverflow ? 5 : max(1, $mobilePrimaryItems->count())); ?>, minmax(0, 1fr));">
        <?php $__currentLoopData = $mobilePrimaryItems; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <?php
                $isActive = $mobileIsItemActive((array) $item);
                $isHighlight = (bool) ($item['highlight'] ?? false);
            ...
            ?>
            <a href="<?php echo e(route($item['route'], $item['params'] ?? [])); ?>" class="mnav-btn <?php echo e(trim($btnClass)); ?>">
                <span><?php echo e($mobileShortLabel((array) $item)); ?></span>
                <?php if($mobileIsSupportChatInbox((array) $item)): ?>
                    <span data-support-chat-badge
                          data-count="<?php echo e($mobileSupportChatUnread); ?>"
                          class="mnav-chat-badge <?php echo e($mobileSupportChatUnread > 0 ? '' : 'hidden'); ?>">
        ...
        <?php if($mobileHasOverflow): ?>
            <button id="mnav-more-open"
                    ...
                    class="mnav-btn <?php echo e($mobileOverflowActive ? 'mnav-btn-active' : ''); ?>">
                <span>Más</span>
            </button>
        <?php endif; ?>
    </div>
