    ...
    <x-ui.card title="Sesion #{{ $session->id }}" subtitle="Apertura {{ $session->opened_at?->format('Y-m-d H:i') }} por {{ $session->openedBy?->name ?? 'N/D' }}">
        ...
        <div class="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
            <article class="rounded-xl border border-slate-200 bg-slate-50 p-3">
                ...
                <p class="mt-1 text-2xl font-black text-slate-900">{{ $currencyFormatter::format((float) $session->opening_balance, $appCurrencyCode) }}</p>
            </article>
            ...
            <table class="ui-table min-w-[640px]">
                ...
                @forelse ($methodTotals as $methodTotal)
                    <tr class="border-b border-slate-100 text-sm">
                        ...
                        <td class="px-3 py-3 text-emerald-700">{{ $currencyFormatter::format((float) $methodTotal->income_total, $appCurrencyCode) }}</td>
                        <td class="px-3 py-3 text-rose-700">{{ $currencyFormatter::format((float) $methodTotal->expense_total, $appCurrencyCode) }}</td>
                    </tr>
                @empty
                    <tr>
