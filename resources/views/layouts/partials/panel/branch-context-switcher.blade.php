@if ($showBranchContextSwitcher)
    ...
    <section class="branch-context-shell">
        <details class="group branch-context-details" @if ($branchContextExpandedByDefault) open @endif>
                    ...
                    <div class="branch-context-summary-main">
                        <div class="branch-context-summary-copy">
                            ...
                            <div class="branch-context-summary-row">
                                <div class="branch-context-summary-copy">
                                    <h2 class="branch-context-title">{{ $activeBranchContextTitle }}</h2>
                                    <p class="branch-context-copy">{{ $activeContextSummary }}</p>
                                </div>
                                <div class="branch-context-pills">
                                    <span class="branch-context-kind">{{ $activeBranchContextBadge ?? 'Activa' }}</span>
                                    <span class="branch-context-status">{{ $activeContextStatus }}</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="branch-context-summary-meta">
                            <div class="branch-context-summary-stats">
                                <span class="branch-context-stat">
                                    <span class="branch-context-stat-figure">{{ $networkGymCount }}</span>
                                    <span class="branch-context-stat-label">Sedes visibles</span>
                                </span>
                                <span class="branch-context-stat">
                                    <span class="branch-context-stat-figure">{{ $linkedBranchCount }}</span>
                            ...
                            <p class="branch-context-location">
                                ...
                                <span>{{ $activeBranchContextAddress }}</span>
                            </p>
                        </div>
                    </div>
                ...
                <div class="branch-context-grid">
                    @if (($showGlobalBranchOption ?? true))
                        <a href="{{ $globalContextUrl }}"
                           class="branch-context-card block is-global-card {{ $isGlobalContextCardActive ? 'is-active' : '' }}"
                           @if ($isGlobalContextCardActive) aria-current="page" @endif>
                            ...
                            <div class="branch-context-card-footer">
                                <p class="branch-context-meta">{{ $linkedBranchCount }} sucursal(es) vinculadas + sede principal</p>
                                <span class="branch-context-card-action">
                                    {{ $isGlobalContextCardActive ? 'Viendo red' : 'Abrir vista' }}
                                    <svg viewBox="0 0 20 20" fill="none" aria-hidden="true">
                    ...
                    @foreach ($branchContextOptions as $branchOption)
                        ...
                        <a href="{{ (string) ($branchOption['url'] ?? '#') }}"
                           class="branch-context-card block {{ $isActiveBranchOption ? 'is-active' : '' }}"
                           @if ($isActiveBranchOption) aria-current="page" @endif>
                            <div class="branch-context-card-head">
                                <div class="branch-context-card-head-main">
                                    <span class="branch-context-card-icon" aria-hidden="true">
                                        @if ($branchKind === 'hub')
                                ...
                                <div class="branch-context-card-badges">
                                    <span class="branch-context-kind">{{ $branchKindLabel }}</span>
                                    @if ($isActiveBranchOption)
                                        <span class="branch-context-status">Actual</span>
                                    @endif
                                </div>
                            </div>
                            ...
                            <div class="branch-context-card-body">
                                <p class="branch-context-card-label">{{ $branchCardLabel }}</p>
                                <h3 class="branch-context-card-heading">{{ (string) ($branchOption['name'] ?? 'Sucursal') }}</h3>
                                <p class="branch-context-card-summary">
                                    {{ (string) ($branchOption['summary'] ?? 'Cambia el contexto operativo de esta sede.') }}
                                </p>
                            </div>

                            <div class="branch-context-card-footer">
                                <p class="branch-context-meta">{{ (string) ($branchOption['address'] ?? '-') }}</p>
                                <span class="branch-context-card-action">
                                    {{ $branchActionLabel }}
                                    <svg viewBox="0 0 20 20" fill="none" aria-hidden="true">
                                        <path d="M7 5l6 5-6 5" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"/>
