...
@if ($showQuickClientAction || $showQuickSaleAction || $renderQuickClientModal || $renderQuickSaleModal)
    <style>
            .quick-access-fab {
                position: fixed;
                z-index: 88;
                ...
                min-width: 3.55rem;
                border-radius: 999px;
                border: 1px solid rgb(15 23 42 / 0.24);
                background: linear-gradient(180deg, rgb(30 41 59), rgb(15 23 42));
                color: #fff;
                padding: 0;
                box-shadow:
                    0 18px 30px -18px rgb(15 23 42 / 0.48),
            ...
            .quick-access-fab:hover {
                ...
                box-shadow:
                    0 22px 34px -18px rgb(15 23 42 / 0.58),
            ...
            .quick-access-modal-shell {
                ...
                background: #0b1220;
                color: #e5eefb;
                box-shadow: 0 34px 64px -38px rgb(15 23 42 / 0.62);
            }

            .quick-access-modal-shell > form,
            .quick-access-modal-shell > div,
            .quick-access-modal-shell header,
            .quick-access-modal-shell .ui-modal-scroll-body {
                background: #0b1220;
            }
            ...
            .quick-access-sale-scan {
                border-radius: 1rem;
                border: 1px solid rgb(8 145 178 / 0.42);
                background: #0b2940;
                padding: 1rem;
            }
            ...
            .quick-access-modal-shell .ui-input,
            .quick-access-modal-shell select.ui-input,
            .quick-access-modal-shell textarea.ui-input,
            .quick-access-modal-shell input.ui-input {
                background: #0f172a !important;
                border-color: rgb(51 65 85) !important;
                color: #f8fafc !important;
            }

            .quick-access-modal-shell .ui-input::placeholder,
            .quick-access-modal-shell textarea.ui-input::placeholder {
                color: #94a3b8;
                opacity: 1;
            }

            .quick-access-modal-shell [data-client-doc-status],
            .quick-access-modal-shell [data-client-account-panel],
            .quick-access-modal-shell [data-client-membership-panel],
            .quick-access-modal-shell #quick-sale-preview,
            .quick-access-modal-shell #quick-sale-list,
            .quick-access-modal-shell #quick-sale-feedback {
    ...
    @push('scripts')
        <script>
            (function () {
                ...
                const quickAccessStoragePrefix = ['gym-quick-access-v5', @json($activeGymSlug), String(@json($activeGymId || 0))].join(':');
                ...
                function bottomClearance() {
                    ...
                    return window.innerWidth < 1024 ? 112 : 22;
                }
                ...
                function storageKeyFor(element) {
                    return [quickAccessStoragePrefix, viewportMode(), element.dataset.quickAction || 'fab'].join(':');
                }
                ...
                function reClampFabs(forceRestore) {
                    const nextViewportMode = viewportMode();
                    const modeChanged = activeViewportMode !== '' && activeViewportMode !== nextViewportMode;
                    activeViewportMode = nextViewportMode;
                ...
                const quickClientConfig = {
                    enabled: Boolean(@json($renderQuickClientModal)),
                    documentCheckUrl: @json($renderQuickClientModal ? route('clients.check-document', $quickActionRouteParams) : ''),
                    plans: @json($quickClientPlanCatalog),
                    promotions: @json($quickClientPromotionCatalog),
                    canManageAccounts: Boolean(@json($quickClientCanManageAccounts)),
                };
                ...
                const quickSaleConfig = {
                    enabled: Boolean(@json($renderQuickSaleModal)),
                    products: @json($quickSaleProductsPayload),
                    storageKey: ['quick-sale-cart', @json($activeGymSlug), String(@json($activeGymId || 0))].join(':'),
                };
...
@if ($renderQuickClientModal)
    <div id="quick-client-modal"
         ...
         data-auto-open="{{ $quickClientAutoOpen ? '1' : '0' }}">
        <div class="ui-modal-shell quick-access-modal-shell w-full max-w-3xl" role="dialog" aria-modal="true" aria-labelledby="quick-client-title">
            <form method="POST"
                  action="{{ route('clients.store', $quickActionRouteParams) }}"
                  enctype="multipart/form-data"
                  novalidate
                  id="quick-client-form"
                  class="flex h-full min-h-0 flex-1 flex-col space-y-0">
                @csrf
                <input type="hidden" name="redirect_to" value="{{ $quickActionReturnUrl }}">
                ...
                <div class="ui-modal-scroll-body space-y-5 px-5 py-5">
                    @if ($quickClientErrorMessages->isNotEmpty())
                        <div class="quick-access-summary-list is-visible rounded-xl border border-rose-500/40 bg-rose-500/10 px-3 py-2 text-sm text-rose-200">
                            ...
                            <ul>
                                @foreach ($quickClientErrorMessages as $message)
                                    <li>{{ $message }}</li>
                                @endforeach
                            </ul>
                        </div>
                    ...
                    <div class="grid gap-4 md:grid-cols-2">
                        <label class="space-y-1 text-sm font-semibold text-slate-300">
                            ...
                            <p class="text-xs font-semibold text-rose-300 {{ $errors->has('first_name') ? '' : 'hidden' }}"
                               data-client-error="first_name">{{ $errors->first('first_name') }}</p>
                        </label>
                        ...
                        <label class="space-y-1 text-sm font-semibold text-slate-300">
                            ...
                            <p class="text-xs font-semibold text-rose-300 {{ $errors->has('last_name') ? '' : 'hidden' }}"
                               data-client-error="last_name">{{ $errors->first('last_name') }}</p>
                        </label>
                        ...
                        <label class="space-y-1 text-sm font-semibold text-slate-300 md:col-span-2">
                            ...
                            <p class="text-xs font-semibold text-rose-300 {{ $errors->has('document_number') ? '' : 'hidden' }}"
                               data-client-error="document_number">{{ $errors->first('document_number') }}</p>
                        ...
                        <label class="space-y-1 text-sm font-semibold text-slate-300">
                            ...
                            <p class="text-xs font-semibold text-rose-300 {{ $errors->has('phone') ? '' : 'hidden' }}"
                               data-client-error="phone">{{ $errors->first('phone') }}</p>
                        </label>

                        <label class="space-y-1 text-sm font-semibold text-slate-300">
                            <span>Género</span>
                            <select name="gender" class="ui-input" data-client-field="gender">
                                <option value="male" @selected(old('gender', 'neutral') === 'male')>Hombre</option>
                                <option value="female" @selected(old('gender', 'neutral') === 'female')>Mujer</option>
                                <option value="neutral" @selected(old('gender', 'neutral') === 'neutral')>Neutral</option>
                            </select>
                            <p class="text-xs font-semibold text-rose-300 {{ $errors->has('gender') ? '' : 'hidden' }}"
                               data-client-error="gender">{{ $errors->first('gender') }}</p>
                        </label>
                        ...
                        <div class="space-y-2 md:col-span-2">
                            ...
                            <p class="text-xs font-semibold text-rose-300 {{ $errors->has('photo') ? '' : 'hidden' }}"
                               data-client-error="photo">{{ $errors->first('photo') }}</p>
                        ...
                        @if ($quickClientCanManageAccounts)
                            <div class="space-y-3 rounded-xl border border-cyan-500/30 bg-cyan-500/5 p-3 md:col-span-2">
                                <div class="flex flex-wrap items-center justify-between gap-2">
                                    <label class="inline-flex items-center gap-2 text-sm font-semibold text-slate-100">
                                        <input type="checkbox"
                                               ...
                                               value="1"
                                               @checked(old('create_app_account'))
                                ...
                                <div class="grid gap-3 md:grid-cols-2 {{ old('create_app_account') ? '' : 'hidden' }}"
                                     data-client-account-panel>
                                    <label class="space-y-1 text-sm font-semibold text-slate-300">
                                        <span>Usuario app</span>
                                        <input type="text"
                                               name="app_username"
                                               value="{{ old('app_username', '') }}"
                                               autocomplete="off"
                                               class="ui-input"
                                               data-client-field="app_username">
                                        <p class="text-xs font-semibold text-rose-300 {{ $errors->has('app_username') ? '' : 'hidden' }}"
                                           data-client-error="app_username">{{ $errors->first('app_username') }}</p>
                                    </label>
                                    ...
                                    <label class="space-y-1 text-sm font-semibold text-slate-300">
                                        ...
                                        <p class="text-xs font-semibold text-rose-300 {{ $errors->has('app_password') ? '' : 'hidden' }}"
                                           data-client-error="app_password">{{ $errors->first('app_password') }}</p>
                                    </label>
                                    ...
                                    <label class="space-y-1 text-sm font-semibold text-slate-300 md:col-span-2">
                                        ...
                                        <p class="text-xs font-semibold text-rose-300 {{ $errors->has('app_password_confirmation') ? '' : 'hidden' }}"
                                           data-client-error="app_password_confirmation">{{ $errors->first('app_password_confirmation') }}</p>
                                    </label>
                                </div>
                            </div>
                        @endif
                    ...
                    <div class="quick-access-panel p-4">
                        ...
                        <div class="quick-access-membership-grid mt-4 {{ old('start_membership') ? '' : 'hidden' }}"
                            ...
                            <div class="grid gap-4 md:grid-cols-2">
                                <label class="space-y-1 text-sm font-semibold text-slate-300">
                                    ...
                                    <select name="plan_id" class="ui-input" data-client-field="plan_id">
                                        ...
                                        @foreach ($quickClientPlans as $plan)
                                            <option value="{{ $plan->id }}" @selected((string) old('plan_id', '') === (string) $plan->id)>
                                                {{ $plan->name }} ({{ \App\Support\PlanDuration::label($plan->duration_unit, (int) $plan->duration_days, $plan->duration_months) }})
                                            </option>
                                        @endforeach
                                    </select>
                                    <p class="text-xs font-semibold text-rose-300 {{ $errors->has('plan_id') ? '' : 'hidden' }}"
                                       data-client-error="plan_id">{{ $errors->first('plan_id') }}</p>
                                </label>
                                ...
                                <label class="space-y-1 text-sm font-semibold text-slate-300">
                                    ...
                                    <input type="date"
                                           ...
                                           value="{{ old('membership_starts_at', now()->toDateString()) }}"
                                           class="ui-input"
                                           data-client-field="membership_starts_at">
                                    <p class="text-xs font-semibold text-rose-300 {{ $errors->has('membership_starts_at') ? '' : 'hidden' }}"
                                       data-client-error="membership_starts_at">{{ $errors->first('membership_starts_at') }}</p>
                                </label>
                                ...
                                <label class="space-y-1 text-sm font-semibold text-slate-300">
                                    ...
                                    <input type="number"
                                           ...
                                           min="0"
                                           step="0.01"
                                           value="{{ old('membership_price', '') }}"
                                           class="ui-input"
                                           data-client-field="membership_price">
                                    <p class="text-xs font-semibold text-rose-300 {{ $errors->has('membership_price') ? '' : 'hidden' }}"
                                       data-client-error="membership_price">{{ $errors->first('membership_price') }}</p>
                                </label>
                                ...
                                <label class="space-y-1 text-sm font-semibold text-slate-300">
                                    ...
                                    <select name="promotion_id"
                                            ...
                                            @disabled(! $quickClientCanManagePromotions)>
                                        <option value="">Sin promoción</option>
                                    </select>
                                    <p class="text-xs font-semibold text-rose-300 {{ $errors->has('promotion_id') ? '' : 'hidden' }}"
                                       data-client-error="promotion_id">{{ $errors->first('promotion_id') }}</p>
                                    @unless ($quickClientCanManagePromotions)
                                ...
                                <label class="space-y-1 text-sm font-semibold text-slate-300">
                                    ...
                                    <p class="text-xs font-semibold text-rose-300 {{ $errors->has('payment_method') ? '' : 'hidden' }}"
                                       data-client-error="payment_method">{{ $errors->first('payment_method') }}</p>
                                </label>
                                ...
                                <label class="space-y-1 text-sm font-semibold text-slate-300">
                                    ...
                                    <p class="text-xs font-semibold text-rose-300 {{ $errors->has('amount_paid') ? '' : 'hidden' }}"
                                       data-client-error="amount_paid">{{ $errors->first('amount_paid') }}</p>
                                </label>
                            </div>
                            ...
                            <p class="rounded-lg border border-amber-500/40 bg-amber-500/10 p-2 text-xs text-amber-200 {{ $quickClientPlans->isEmpty() ? '' : 'hidden' }}"
                               data-client-no-plans>
                                No hay planes activos. Crea un plan antes de iniciar membresías desde este modal.
                            </p>

                            <div class="rounded-xl border-2 border-rose-400/80 bg-rose-500/20 p-3 text-rose-100 shadow-lg {{ $errors->has('cash') ? '' : 'hidden' }}"
                                 data-client-cash-alert>
                                <p class="text-xs font-black uppercase tracking-wide">Debe abrir caja para cobrar</p>
                                <p class="mt-1 text-sm font-semibold">{{ $errors->first('cash') }}</p>
                                <div class="mt-2">
                                    <x-ui.button :href="route('cash.index', $quickActionRouteParams)" variant="secondary" size="sm">Ir a caja</x-ui.button>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <footer class="ui-modal-sticky-footer flex items-center justify-end gap-3 px-5 py-4">
                    <x-ui.button type="button" variant="ghost" data-quick-client-close>Cancelar</x-ui.button>
...
@if ($renderQuickSaleModal)
    <div id="quick-sale-modal"
         class="ui-modal-backdrop hidden items-start"
         data-auto-open="{{ $quickSaleAutoOpen ? '1' : '0' }}">
        <div class="ui-modal-shell quick-access-modal-shell quick-access-sale-panel w-full max-w-5xl" role="dialog" aria-modal="true" aria-labelledby="quick-sale-title">
            <div class="flex h-full min-h-0 flex-1 flex-col">
                ...
                <div class="ui-modal-scroll-body space-y-5 px-5 py-5">
                    @if ($quickSaleErrorMessages->isNotEmpty())
                        <div class="quick-access-summary-list is-visible rounded-xl border border-rose-500/40 bg-rose-500/10 px-3 py-2 text-sm text-rose-200">
                            <p class="font-semibold">Revisa estos puntos antes de registrar la venta:</p>
                            <ul>
                                @foreach ($quickSaleErrorMessages as $message)
                                    <li>{{ $message }}</li>
                                @endforeach
                            </ul>
                        </div>
                    ...
                    <div class="quick-access-sale-shell p-4">
                        ...
                        @elseif (! $quickSaleHasOpenCashSession)
                            <p class="ui-alert ui-alert-warning">Caja cerrada. Debes abrir caja antes de registrar ventas de productos.</p>
                            <div class="mt-3">
                                <x-ui.button :href="route('cash.index', $quickActionRouteParams)" variant="secondary">Ir a caja</x-ui.button>
                            </div>
                        @elseif ($quickSaleProducts->isEmpty())
                            <p class="ui-alert ui-alert-warning">No hay productos activos para vender. Crea al menos un producto y carga stock primero.</p>
                            <div class="mt-3">
                                <x-ui.button :href="route('products.index', $quickActionRouteParams)" variant="secondary">Ir a productos</x-ui.button>
                            </div>
                        @else
                            <form method="POST"
                                  action="{{ route('sales.store', $quickActionRouteParams) }}"
                                ...
                                <input type="hidden" name="sale_items_payload" id="quick-sale-items-payload" value="{{ old('sale_items_payload', '') }}">
                                <input type="hidden" name="quantity" id="quick-sale-quantity-input" value="{{ old('quantity', 1) }}">
                                ...
                                <div class="quick-access-sale-scan md:col-span-2">
                                    <div class="grid gap-3 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-end">
                                        ...
                                        <x-ui.button type="button" id="quick-sale-scan-search" variant="secondary" class="w-full lg:w-auto">Buscar código</x-ui.button>
                                    </div>
                                ...
                                <label class="space-y-1 text-sm font-semibold ui-muted md:col-span-2">
                                    ...
                                    <select name="product_id" id="quick-sale-product-select" class="ui-input">
                                        ...
                                        @foreach ($quickSaleProducts as $product)
                                            <option value="{{ $product->id }}" @selected($quickSaleSelectedProductId === (int) $product->id)>
                                                {{ $product->name }} | SKU {{ $product->sku ?: '---' }} | BAR {{ $product->barcode ?: '---' }} | stock {{ (int) $product->stock }} | {{ $quickActionCurrencyFormatter::format((float) $product->sale_price, $quickActionCurrencyCode) }}
                                            </option>
                                        @endforeach
                                    </select>
                                    ...
                                    <p class="text-xs font-semibold text-rose-300 {{ $errors->has('product_id') ? '' : 'hidden' }}">{{ $errors->first('product_id') }}</p>
                                </label>
                                ...
                                <label class="space-y-1 text-sm font-semibold ui-muted md:col-span-2">
                                    ...
                                    <p class="text-xs font-semibold text-rose-300 {{ $errors->has('payment_method') ? '' : 'hidden' }}">{{ $errors->first('payment_method') }}</p>
                                </label>
                                ...
                                <label class="space-y-1 text-sm font-semibold ui-muted md:col-span-2">
                                    ...
                                    <select name="client_id" class="ui-input">
                                        ...
                                        @foreach ($quickSaleClients as $client)
                                            <option value="{{ $client->id }}" @selected($quickSaleSelectedClientId === (int) $client->id)>
                                                {{ $client->full_name }}
                                            </option>
                                        @endforeach
                                    </select>
                                    <p class="text-xs font-semibold text-rose-300 {{ $errors->has('client_id') ? '' : 'hidden' }}">{{ $errors->first('client_id') }}</p>
                                </label>

                                <label class="space-y-1 text-sm font-semibold ui-muted md:col-span-2">
                                    <span>Notas</span>
                                    <textarea name="notes" rows="3" class="ui-input" placeholder="Ej: bebida, guantes, proteína, promo del día">{{ old('notes') }}</textarea>
                                    <p class="text-xs font-semibold text-rose-300 {{ $errors->has('notes') ? '' : 'hidden' }}">{{ $errors->first('notes') }}</p>
                                </label>

                                <div class="quick-access-sale-actions md:col-span-2">
                                    <x-ui.button type="submit">Registrar venta</x-ui.button>
                                    <x-ui.button :href="route('products.index', $quickActionRouteParams)" variant="ghost">Abrir productos</x-ui.button>
                                    <x-ui.button type="button" variant="ghost" data-quick-sale-close>Cancelar</x-ui.button>
                                </div>
                            </form>
                        @endif
                    </div>
