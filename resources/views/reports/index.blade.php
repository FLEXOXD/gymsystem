...
@section('content')
    ...
    <div class="report-hub space-y-4">
        ...
        @if ($planControlReportsDashboard)
            @php
            ...
            <section class="report-control-shell">
                <div class="report-control-grid">
                    <div class="report-control-copy">
                        ...
                        <h2 class="report-control-heading">{{ $planControlReportsDashboard['headline'] ?? 'Reportes listos para leer la sede sin ruido' }}</h2>
                        <p class="report-control-summary">{{ $planControlReportsDashboard['summary'] ?? 'Caja, membresias y asistencia quedan ordenadas en una sola lectura ejecutiva.' }}</p>
                    </div>

                    <div class="report-control-actions">
                        <x-ui.button :href="route('reports.income', ['contextGym' => $contextGym] + request()->query())" variant="primary">Detalle ingresos</x-ui.button>
                        <x-ui.button :href="route('reports.memberships', $membershipsRouteParams)" variant="secondary">Estado membresias</x-ui.button>
                    ...
                    <div class="report-control-priority-grid">
                        @foreach ($planControlPriorities as $priority)
                            <article class="report-control-priority" data-tone="{{ $priority['tone'] }}">
                                <p class="report-control-priority-label">{{ $priority['label'] }}</p>
                                <p class="report-control-priority-value">{{ $priority['value'] }}</p>
                                <p class="report-control-priority-note">{{ $priority['note'] }}</p>
                            </article>
                        @endforeach
                    </div>
                </div>
        ...
        @if ($professionalReportsDashboard)
            @php
                ...
                <div class="report-pro-grid">
                    <div class="report-elite-head flex flex-wrap items-start justify-between gap-3">
                        <div class="report-pro-copy">
                            ...
                            <h2 class="report-pro-heading">{{ $professionalReportsDashboard['headline'] ?? 'Radar de crecimiento' }}</h2>
                            <p class="report-pro-summary">{{ $professionalReportsDashboard['summary'] ?? 'Membresias, productos y promos dentro de una misma lectura ejecutiva.' }}</p>
                        </div>
                        <span class="report-pro-badge">Profesional</span>
                    </div>

                    <div class="report-pro-metrics">
                        @foreach ($professionalReportMetrics as $metric)
                            <article class="report-pro-metric" data-tone="{{ $metric['tone'] }}">
                                <p class="report-pro-metric-label">{{ $metric['label'] }}</p>
                                <p class="report-pro-metric-value">{{ $metric['value'] }}</p>
                                <p class="report-pro-metric-note">{{ $metric['note'] }}</p>
                            </article>
                        @endforeach
                    </div>

                    @if ($professionalReportAlerts->isNotEmpty())
                        <div class="report-pro-insights">
                            @foreach ($professionalReportAlerts as $alert)
                                <article class="report-pro-chip" data-tone="{{ $alert['tone'] ?? 'info' }}">
                                    <p class="report-pro-chip-title">{{ $alert['title'] ?? 'Alerta' }}</p>
                                    <p class="report-pro-chip-copy">{{ $alert['description'] ?? '' }}</p>
                                </article>
                            @endforeach
                        </div>
                    @endif
                    ...
                    <div class="report-pro-actions">
                        <x-ui.button :href="route('reports.client-earnings', $clientEarningsRouteParams)" variant="primary">Ganancias de clientes</x-ui.button>
                        <x-ui.button :href="route('clients.index', $expiringClientsRouteParams)" variant="secondary">Ver por vencer</x-ui.button>
                        <x-ui.button :href="route('plans.index', $baseRouteParams)" variant="ghost">Planes y promos</x-ui.button>
                        ...
                        @else
                            <x-ui.button :href="route('clients.index', $expiredClientsRouteParams)" variant="ghost">Ver vencidos</x-ui.button>
                        @endif
                    </div>
                </div>
            </section>
        ...
        @if ($premiumReportsDashboard)
            ...
            <section class="report-pro-shell report-elite-shell">
                <div class="report-pro-grid">
                    ...
                    <div class="report-pro-metrics">
                        @foreach ($premiumReportMetrics as $metric)
                            <article class="report-pro-metric" data-tone="{{ $metric['tone'] }}">
                                <p class="report-pro-metric-label">{{ $metric['label'] }}</p>
                                <p class="report-pro-metric-value">{{ $metric['value'] }}</p>
                                <p class="report-pro-metric-note">{{ $metric['note'] }}</p>
                    ...
                    <div class="report-pro-actions">
                        @if ($canManageClientAccounts && \Illuminate\Support\Facades\Route::has('client-portal.index'))
                            <x-ui.button :href="route('client-portal.index', $baseRouteParams)" variant="primary">Portal cliente</x-ui.button>
                        @endif
                        <x-ui.button :href="route('reports.client-earnings', $clientEarningsRouteParams)" variant="secondary">Ganancias de clientes</x-ui.button>
                        @if ($canUseSalesInventoryReports)
                            <x-ui.button :href="route('reports.sales-inventory', ['contextGym' => $contextGym] + request()->query())" variant="ghost">Ventas e inventario</x-ui.button>
                        @else
                            <x-ui.button :href="route('reports.income', ['contextGym' => $contextGym] + request()->query())" variant="ghost">Detalle ingresos</x-ui.button>
                        @endif
                    </div>
                </div>
            </section>
        ...
        <x-ui.card class="report-hub-filter" title="Panel de reportes" :subtitle="$reportFilterSubtitle">
                ...
                <div class="report-filter-toolbar">
                    ...
                    @else
                        <p class="text-xs font-semibold text-amber-700 dark:text-amber-300">
                            {{ $isBranchContext ? 'Sucursal secundaria: exportacion bloqueada (solo lectura).' : 'Este plan mantiene lectura en pantalla; PDF y CSV se habilitan al subir a Profesional.' }}
                        </p>
                    @endif
                </div>
            </form>
        ...
        <section class="report-module-grid grid gap-4 {{ $moduleGridClass }}">
        ...
        <section class="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            <x-ui.card class="report-kpi-card" data-tone="income">
            ...
            <x-ui.card class="report-kpi-card" data-tone="expense">
            ...
            <x-ui.card class="report-kpi-card" data-tone="balance">
            ...
            <x-ui.card class="report-kpi-card" data-tone="motion">
            ...
            <x-ui.card class="report-kpi-card" data-tone="attendance">
            ...
            <x-ui.card class="report-kpi-card" data-tone="membership">
        ...
        <x-ui.card class="report-nav-card" title="Navegación rápida de reportes">
